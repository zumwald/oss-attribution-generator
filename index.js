...
function getNpmLicenses() {
  ...
  return bluebird
    ...
    .then(result => {
      ...
      var topLevelProjectInfo = jetpack.read(
        path.join(dir, "package.json"),
        "json"
      );
      var keys = Object.getOwnPropertyNames(result).filter(k => {
        return (
          k !== `${topLevelProjectInfo.name}@${topLevelProjectInfo.version}`
        );
      });
      ...
      return bluebird.map(
        ...
        key => {
          ...
          var defaultPackagePath = `${package["dir"]}/node_modules/${
            package.name
          }/package.json`;

          let itemAtPath = jetpack.exists(defaultPackagePath);
          ...
          if (packagePath && packagePath[0]) {
            ...
            return Promise.reject(
              `${package.name}: unable to locate package.json`
            );
          }
          ...
          props.authors =
            (packageJson.author &&
              getAttributionForAuthor(packageJson.author)) ||
          ...
          let licenseFile = package.licenseFile;
    ...
    .then(result => {
      ...
      return bluebird.map(
        ...
        component => {
          ...
          var licenses =
            package.license && _.isString(package.license)
              ? package.license
          ...
          if (licensePath) {
            licenseText = jetpack.read(
              path.join(bowerComponentsDir, licensePath.relativePath)
            );
          }
...
taim(
  "Total Processing",
  bluebird.all([taim("Npm Licenses", getNpmLicenses()), getBowerLicenses()])
)
  .catch(err => {
    console.log(err);
    process.exit(1);
  ...
  .then(licenseInfos => {
    ...
    var attribution = attributionSequence.join(
